...
Die Team-Zuordnung wird in `users/{uid}.teamIds` gespeichert; es gibt keine separate Collection `userTeams` und keine Subcollection `users/{uid}/teams`. Beim Speichern einer Zuordnung erstellt die Admin-Ansicht zusätzlich die gegenseitigen `teamAccess/{viewerUid}_{targetUid}`-Dokumente für alle User-Paare innerhalb jedes Teams. Team-Vergleich und Live-Trainings laden zuerst `teamAccess` mit `viewerId == currentUser.uid`, ergänzen die eigene UID und laden anschließend Workouts sowie User-Teamdaten aus `users/{uid}` nur für genau diese berechtigten User. Neue Trainings speichern zusätzlich die aktuellen `teamIds` des Nutzers.
